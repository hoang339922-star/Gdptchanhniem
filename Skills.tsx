...
const Skills = () => {
  ...
  const handleTopicSelect = (topic: string) => {
    ...
    // Define which topics have dedicated Lesson Views
    const hasSpecialLesson = ['Semaphore', 'Morse', 'DauDiDuong', 'CuuThuong'].includes(selectedTopic);

    if (hasSpecialLesson) {
        setIsQuizMode(false); // Go to Lesson View first
    } else {
        // Other topics (General Knowledge, Buddhist Study) go straight to quiz
  ...
  // 3. SPECIAL LESSON VIEWS (Study Mode)
  switch (activeTopic) {
      case 'Semaphore':
          return (
              <SemaphoreLesson 
      ...
      default:
          // Fallback if no specific lesson view exists (should have been caught by startQuiz)
          return (
